...
def build_inputs(
    ...
    residual = torch.randn(
        num_tokens, hc_mult, hidden, device=device, dtype=torch.bfloat16
    )
    ...
    if model is not None:
        ...
        head_base = _load_model_tensor(model, "hc_head_base").to(device, torch.float32)
...
def run_backend(
    ...
    post, comb, layer_input = mod.mhc_pre(
        residual, fn, scale, base, 1e-6, 1e-6, 1e-6, 2.0, 3
    )
