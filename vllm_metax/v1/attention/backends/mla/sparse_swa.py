...
def _validate_dspark_query_lengths(
    query_start_loc_cpu: torch.Tensor, *, max_query_tokens: int
) -> None:
    """Fail before launch if a DSpark query exceeds the fixed index buffer."""
    if query_start_loc_cpu.ndim != 1 or query_start_loc_cpu.numel() < 2:
        raise ValueError("DSpark query start locations must be a 1D offset array")
    max_query_len = int(
        (query_start_loc_cpu[1:] - query_start_loc_cpu[:-1]).max().item()
    )
    if max_query_len > max_query_tokens:
        raise ValueError(
            f"DSpark non-causal query length {max_query_len} exceeds "
            f"allocated maximum {max_query_tokens}"
        )
...
class DeepseekSparseSWAMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # DSpark draft attention is non-causal: each query sees the trailing
        # context window plus every query token in the block, including future
        # tokens. The native consumer pads this list to a multiple of 128.
        self.is_dspark = spec_config is not None and spec_config.use_dspark()
        bonus_anchor = int(bool(getattr(hf_config, "dspark_bonus_anchor", False)))
        self.max_dspark_query_tokens = (
            self.num_speculative_tokens + bonus_anchor if self.is_dspark else 0
        )
        self.noncausal_index_width = (
            (self.window_size + self.max_dspark_query_tokens + 127) // 128 * 128
            if self.is_dspark
            else 0
        )
    ...
    def build(
        ...
        if num_decode_tokens > 0:
            ...
            if non_causal:
                _validate_dspark_query_lengths(
                    query_start_loc_cpu,
                    max_query_tokens=self.max_dspark_query_tokens,
                )
                decode_swa_indices = self._get_noncausal_decode_indices()
    ...
    def _get_noncausal_decode_indices(self) -> torch.Tensor:
        """Return the graph-stable DSpark index buffer, allocating it once."""
        ...
        return self.decode_swa_indices_noncausal
...
def _compute_dspark_noncausal_swa_indices_kernel(
    ...
    """Build DSpark's non-causal trailing-context-plus-query index list."""
    pid = tl.program_id(0)
    ...
    # All tokens in a draft block share the same context window and include
    # every query token, including positions after the current token.
